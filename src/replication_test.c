    ...
    int *zrepl_status_msg_cnt)
{
	...
	for (start = 0; start < iovec_count; start += 1) {
		...
		while (nbytes) {
			rc = writev(fd,
			&iovec[start], 1); // Review iovec in this line
			if (rc < 0) {
				goto out;
			}
			nbytes -= rc;
...
main(int argc, char **argv)
{
	...
	zrepl_status =
	(zrepl_status_ack_t *)malloc(sizeof (zrepl_status_ack_t));
	...
	if ((sfd = cstor_ops.conn_listen(replica_ip, replica_port, 32, 1))
		< 0) {
		...
		for (i = 0; i < event_count; i++) {
			if ((events[i].events & EPOLLERR) ||
				...
				if (retry) {
					/*
					 * If connection with
					 * target is successfully
					 * established then
					 * there is no need
					 * to re-connect if error occurs.
					 */
					retry = false;
				}
