...
main(int argc, char **argv)
{
	...
	zrepl_status = (zrepl_status_ack_t *)
	malloc(sizeof (zrepl_status_ack_t));
	...
	if ((sfd = cstor_ops.conn_listen(replica_ip,
	replica_port, 32, 1)) < 0) {
			REPLICA_LOG("conn_listen() failed, err:%d replica(%d)",
				errno, ctrl_port);
		close(vol_fd);
		destroy_mdlist();
				exit(EXIT_FAILURE);
	...
	while (1) {
		...
		for (i = 0; i < event_count; i++) {
			if ((events[i].events & EPOLLERR) ||
				...
				count = test_read_data(events[i].data.fd,
				...
				if (retry) {
					/*
					 * If connection with target is
					 * successfully established then
					 * there is no need to re-connect
					 * if error occurs.
					 */
					retry = false;
				}
				if (count != sizeof (zvol_io_hdr_t)) {
				REPLICA_ERRLOG("Failed to read complete header..
				got only %ld bytes out of %lu\n",
				...
				if (mgmtio->len) {
					data = data_ptr_cpy =
					malloc(mgmtio->len);
					count = test_read_data(
						events[i].data.fd,
					(uint8_t *)data, mgmtio->len);
					if (count < 0) {
						rc = -1;
						goto error;
					} else if ((uint64_t)count !=
					mgmtio->len) {
				REPLICA_ERRLOG("failed to getch mgmt data..
				got only %ld bytes out of %lu\n",
						    count, mgmtio->len);
						rc = -1;
						goto error;
				...
				send_mgmt_ack(mgmtfd, opcode, data, replica_ip,
				replica_port, zrepl_status,
			...
			errno, ctrl_port);
				...
				if (rc == 0) {
				REPLICA_LOG("Accepted connection on
				descriptor %d "
					"(host=%s, port=%s)\n",
					iofd, hbuf, sbuf);
				}
				...
				while (1) {
					if (read_rem_data) {
					count = test_read_data(events[i].data.
					fd,
				...
				(uint8_t *)io_hdr, io_hdr_len);
					...
					ZVOL_OPCODE_WRITE ||
						...
						if (io_hdr->len) {
							...
							nbytes = 0;
							count =
					test_read_data(events[i].data.fd,
