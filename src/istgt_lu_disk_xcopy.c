...
#define	getdata(data, lu_cmd) {		\
	if (lu_cmd->iobufindx == -1) {		\
		...
		for (_i = 0; _i <= lu_cmd->iobufindx;
			(dptr += lu_cmd->iobuf[_i].iov_len), _i++)\
			memcpy(dptr, lu_cmd->iobuf[_i].iov_base,
				lu_cmd->iobuf[_i].iov_len);\
	}	\
}
...
#define	BUILD_SENSE(SK, ASC, ASCQ) istgt_lu_scsi_build_sense_data
	(lu_cmd, ISTGT_SCSI_SENSE_ ## SK, (ASC), (ASCQ))
...
istgt_find_xcopy_target(ISTGT_Ptr istgt, uint64_t identifier)
{
	...
	for (i = 0; i < MAX_LOGICAL_UNIT; i++) {
			...
			if (lu == NULL)
				continue;
			/* Cloudbyte supports single */
			/* lun for logical group so zero */
...
istgt_get_xcopy_target(ISTGT_LU_DISK *spec,
	uint8_t *target_descriptor, ISTGT_LU_CMD_Ptr lu_cmd)
{
	...
	if (td_opcode != 0xE4) {
		...
		ISTGT_ERRLOG("TARGET_DESCRIPTOR OPCODE NOT SUPPORTED %x \n",
			td_opcode);
		BUILD_SENSE(ILLEGAL_REQUEST,
		0x26, 0x07); // Refer  SCSI Primary Commands - 3 (SPC-3)
		xfree(target);
		return (NULL);
	}
	...
	if (code_set == 1) {
		ISTGT_TRACELOG(ISTGT_TRACE_DEBUG,
			"Association and Identifier field \
	...
	ISTGT_TRACELOG(ISTGT_TRACE_DEBUG,
		"Association %d, Identifier_type %d, Identifier_length %d\n",
	...
	if (! (pdt == PDT_DIRECT_ACCESS_BLK_DEV ||
		pdt == PDT_SIMPLIFIED_DIRECT_ACCESS_DEV)) {
...
int
istgt_lu_disk_process_xcopy(ISTGT_LU_DISK *spec, CONN_Ptr conn,
	ISTGT_LU_CMD_Ptr lu_cmd, uint8_t *data_buf, int dlen)
{
	ISTGT_XCOPY_TGT *src_tgt = NULL,
	*dst_tgt = NULL;
	uint64_t src_lba = 0, dst_lba = 0;
	/* Source and Destination logical block address */
	uint32_t num_of_blks_byts = 0;
	uint8_t *src_td = NULL, *dst_td = NULL;
	/* Source Destination target descriptor */
	uint8_t lstid = 0; /* List identifier */
	uint8_t sd_opcode = 0;
	/* segment Descriptor opcode support only 0x2h, 0xDh, 0xAh */
	uint8_t stdi, dtdi;
	/* Source target descriptor and Destination target descriptor index */
	off_t src_offset = 0, dst_offset = 0;
	int rc = 0; /* result */
	int dc = 0, cat = 0;
	int tdll = 0;
	/* Target and Segment Descriptor List Length */
	int len = 16;
	/* Actual block starts with the offset 16 */
	int str, list_id_usage, priority;
	/* sequential striped (str), No Receive Copy Results (NRCR), Priority */
	int num_tar_desc;
	/* Number of target descriptor */
	// uint32_t transfer_len = 0; /* Transfer length */
	int sdl = 0; /* Segment descriptor length */

	if (spec == NULL) {
	/* This spec can be source target / */
	/* destination target, should not be NULL */
		ISTGT_ERRLOG("c#%d SPEC in the istgt_lu_disk_process_xcopy \
			is NULL\n", conn->id);
		goto fail;
	}
	...
	if (priority == 0) {
		...
		/* CloudByte: Currently does not */
		/* support priority on the commands. */
	}
	...
	if (list_id_usage == LIST_ID_USAGE) {
		...
		/* the copy manager need not hold any */
		/* data for tha application client, */
		/* and the Extended copy commands list */
		/* identifier value should only be zero */
	...
	/* must not be more than MAXIMUM TARGET DESCRIPTOR COUNT */
	...
	if (num_tar_desc != MAX_CSCD_DESCRIPTOR_COUNT) {
		...
		BUILD_SENSE(ILLEGAL_REQUEST,
		0x26, 0x06);	/* Refer spc4r36s */
		goto fail;
	}

	DSET32(&data_buf[4], 0);
	/* Copy manager to ensure bytes 4 through 7 contains zero */

	/* Fetch the segment descriptors */
	/* to get the index for target descriptor */
	len = PARAMETER_HEADER_DATA + tdll;

	// need to come back to this one later..
	// if we need to have xcopy support
	// while (((len + (PARAMETER_HEADER_DATA+tdll+(sdll))) <= dlen)
	// && (len < (PARAMETER_HEADER_DATA+tdll+(sdll))))
	while (len +4 <=  dlen) {
	sd_opcode = DGET8(&data_buf[len]);
		if (!(sd_opcode == SEGMENT_DESCRIPTOR_B2B ||
			sd_opcode == SEGMENT_DESCRIPTOR_B2B_OFFSET)) {
			ISTGT_ERRLOG("c#%d SD OPCODE NOT SUPPORTED %x  \n",
				conn->id, sd_opcode);
			BUILD_SENSE(ILLEGAL_REQUEST, 0x26, 0x09);
			// Refer  SCSI Primary Commands - 3 (SPC-3)
			goto fail;
		}
		...
		if (len+sdl > dlen) {
			ISTGT_ERRLOG("c#%d SD data error we need to send\
				COPY ABORTED %x  \n", conn->id, sd_opcode);
			BUILD_SENSE(ILLEGAL_REQUEST, 0x26, 0x09);
			// Refer  SCSI Primary Commands - 3 (SPC-3)
			goto fail;
		...
		if (src_tgt == NULL) {
			ISTGT_ERRLOG("c#%d Could not find the\
				source target \n", conn->id);
			goto fail;
		}
		...
		rc = istgt_lu_disk_lbxcopy(src_tgt, dst_tgt, conn,
			lu_cmd, dc, cat, num_of_blks_byts, sd_opcode);
		if (rc < 0) {
...
istgt_lu_disk_xcopy(ISTGT_LU_DISK *spec, CONN_Ptr conn, ISTGT_LU_CMD_Ptr lu_cmd)
{
	...
	if (opcode != 0x83) {
		ISTGT_ERRLOG("c#%d EXTENDED COPY command opcode \
			seems to be wrong \n", conn->id);
		return (-1);
	}
...
static uint64_t
istgt_obtain_xcopy_nbytes(ISTGT_XCOPY_TGT *src_tgt,
	ISTGT_XCOPY_TGT *dst_tgt, int dc,
...
int
istgt_lu_disk_lbxcopy(ISTGT_XCOPY_TGT *src_tgt,
	ISTGT_XCOPY_TGT *dst_tgt, CONN_Ptr conn, ISTGT_LU_CMD_Ptr lu_cmd,
	int dc, int cat,  uint64_t num_blks_byts, uint8_t sd_opcode)
{
	...
	if (src_tgt->spec->rsv_key) {
		rc = istgt_lu_disk_check_pr(src_tgt->spec,
			conn, PR_ALLOW(1, 0, 1, 1, 0));
		if (rc != 0) {
	...
	if (dst_tgt->spec->rsv_key) {
		rc = istgt_lu_disk_check_pr(dst_tgt->spec,
			conn, PR_ALLOW(1, 0, 1, 1, 0));
		if (rc != 0) {
			lu_cmd->status = ISTGT_SCSI_STATUS_RESERVATION_CONFLICT;
			return (-1);
	...
	nbytes = istgt_obtain_xcopy_nbytes(src_tgt,
		dst_tgt, dc, num_blks_byts, sd_opcode);
	if (nbytes == 0) {
		/* No data to copy */
		lu_cmd->data_len = 0;
		return (0);
	...
	if (src_tgt->lba >= maxlba || llen > maxlba ||
		src_tgt->lba > (maxlba - llen)) {
...
MTX_LOCK(&src_tgt->spec->clone_mutex);
	...
	if (rc < 0 || (uint64_t) rc != nbytes) {
		xfree(clone_buf);
		/* CloudByte: TODO FIX, Refer spc4rs36 Table-118, */
		/* Preserve the residual data */
	/* for the next segment processing if the cat bit is being Non-zero */
		...
		if (src_tgt->pad  == 0 && dst_tgt->pad == 0 && cat == 0) {
			BUILD_SENSE(COPY_ABORTED,
			0x26, 0x0A);
			/* Copy Aborted: Unexpected Inexact Segment */
		}
	...
	if (dst_tgt->lba >= maxlba ||
		llen > maxlba || dst_tgt->lba > (maxlba - llen)) {
		ISTGT_ERRLOG("c#%d end of media\n", conn->id);
		xfree(clone_buf);
		return (-1);
	}
	...
	rc = pwrite(dst_tgt->spec->fd, clone_buf,
		nbytes, dst_tgt->offset);
	if (rc < 0 || (uint64_t) rc != nbytes) {
		xfree(clone_buf);
		/* CloudByte: TODO FIX, Refer sp4r36s Table-118, */
		/* Preserve the residual data for the next */
		/* segment processing if the cat bit being NON-Zero */
