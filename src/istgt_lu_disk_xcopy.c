...
#define	BUILD_SENSE(SK, ASC, ASCQ) istgt_lu_scsi_build_sense_data(lu_cmd,
    ISTGT_SCSI_SENSE_ ## SK, (ASC), (ASCQ))
	...
	uint8_t *target_descriptor, ISTGT_LU_CMD_Ptr lu_cmd)
{
	...
	if (td_opcode != 0xE4) {
		...
		ISTGT_ERRLOG("TARGET_DESCRIPTOR OPCODE NOT SUPPORTED %x \n",
			td_opcode);
		BUILD_SENSE(ILLEGAL_REQUEST, 0x26, 0x07);
	...
	ISTGT_TRACELOG(ISTGT_TRACE_DEBUG, "Association %d, "
		"Identifier_type %d, Identifier_length %d\n",
	...
	ISTGT_LU_CMD_Ptr lu_cmd, uint8_t *data_buf, int dlen)
{
	...
	uint8_t sd_opcode = 0; /* segment Descriptor opcode */
	/* support only 0x2h, 0xDh, 0xAh */
	uint8_t stdi, dtdi; /* Source target descriptor */
	/* and Destination target descriptor index */
	...
	if (spec == NULL) { /* This spec can be source target / */
	...
	if (list_id_usage == LIST_ID_USAGE) {
		...
		/* the copy manager need not hold any data for, */
		/* the application client and the Extended copy commands list */
	...
	/* Number of target descriptors must */
	/* not be more than MAXIMUM TARGET DESCRIPTOR COUNT */
	...
	if (num_tar_desc != MAX_CSCD_DESCRIPTOR_COUNT) {
		...
		BUILD_SENSE(ILLEGAL_REQUEST, 0x26, 0x06);
		/* Refer spc4r36s */
		goto fail;
	}
	...
	/* Fetch the segment descriptors to get the */
	/* index for target descriptor */
...
istgt_lu_disk_xcopy(ISTGT_LU_DISK *spec, CONN_Ptr conn, ISTGT_LU_CMD_Ptr lu_cmd)
{
	...
	if (opcode != 0x83) {
		ISTGT_ERRLOG("c#%d EXTENDED COPY command "
			"opcode seems to be wrong \n", conn->id);
		return (-1);
	}
...
static uint64_t
istgt_obtain_xcopy_nbytes(ISTGT_XCOPY_TGT *src_tgt, ISTGT_XCOPY_TGT *dst_tgt,
	int dc, uint64_t num_blks_byts, uint8_t sd_opcode)
{
	uint64_t nbytes = 0;
	if (sd_opcode == 0x02) {
...
int
istgt_lu_disk_lbxcopy(ISTGT_XCOPY_TGT *src_tgt, ISTGT_XCOPY_TGT *dst_tgt,
	...
	int dc, int cat,  uint64_t num_blks_byts, uint8_t sd_opcode)
{
	...
	if (src_tgt->spec->rsv_key) {
		rc = istgt_lu_disk_check_pr(src_tgt->spec, conn,
			PR_ALLOW(1, 0, 1, 1, 0));
		if (rc != 0) {
	...
	if (dst_tgt->spec->rsv_key) {
		rc = istgt_lu_disk_check_pr(dst_tgt->spec, conn,
			PR_ALLOW(1, 0, 1, 1, 0));
		if (rc != 0) {
			lu_cmd->status = ISTGT_SCSI_STATUS_RESERVATION_CONFLICT;
			return (-1);
	...
	nbytes = istgt_obtain_xcopy_nbytes(src_tgt, dst_tgt, dc,
		num_blks_byts, sd_opcode);
	if (nbytes == 0) {
		/* No data to copy */
		lu_cmd->data_len = 0;
		return (0);
	...
	if (rc < 0 || (uint64_t) rc != nbytes) {
		...
		/* segment processing if the cat bit is being Non-zero */
		MTX_UNLOCK(&src_tgt->spec->clone_mutex);
		if (src_tgt->pad  == 0 && dst_tgt->pad == 0 && cat == 0) {
			BUILD_SENSE(COPY_ABORTED, 0x26, 0x0A);
	...
	if (dst_tgt->lba >= maxlba || llen > maxlba ||
		dst_tgt->lba > (maxlba - llen)) {
		ISTGT_ERRLOG("c#%d end of media\n",
			conn->id);
		xfree(clone_buf);
		return (-1);
	...
	if (rc < 0 || (uint64_t) rc != nbytes) {
		xfree(clone_buf);
	/* CloudByte: TODO FIX, */
	/* Refer sp4r36s Table-118, Preserve the residual data for */
	/* the next segment processing if the cat bit being NON-Zero */
