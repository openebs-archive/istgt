...
ISTGT_INTEGRATION=$DIR/src/istgt_integration.full
ISCSIADM=iscsiadm
...
wait_for_healthy_volume() {
	while [ 1 ]; do
		istgtcontrol snapcreate vol1 snap1 30
		if [ $? -eq 0 ]; then
			break
		fi
		sleep 5
...
run_data_integrity_test() {
	...
	$REPLICATION_TEST -i "$CONTROLLER_IP" -p "$CONTROLLER_PORT" -I "$replica1_ip" -P "$replica1_port" -V "/tmp/test_vol1" -n 100&
	replica1_pid1=$!
	sleep 5
	write_and_verify_data
	sleep 5
